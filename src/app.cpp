#include <ros/ros.h>

#include <Eigen/Dense>
#include <vector>

#include <pcpred/pcpred.h>

#include <iostream>

using namespace pcpred;


typedef std::vector<Eigen::Vector3d> Path;


Path produceObstaclePathStatic(int frame_count)
{
    Path path;

    for (int i=0; i<frame_count; i++)
        path.push_back(Eigen::Vector3d(0, 0, 0));

    return path;
}

Path produceObstaclePathSine(int frame_count, double speed)
{
    Path path;

    for (int i=0; i<frame_count; i++)
    {
        double t = speed * i;
        path.push_back(Eigen::Vector3d(t, sin(t), 0));
    }

    return path;
}


int main(int argc, char** argv)
{
    ros::init(argc, argv, "app_pointcloud_prediction");

    ROS_INFO("app_pointcloud_prediction");

    PointPredictor predictor;
    predictor.setTimestep(0.1);
    predictor.setSensorDiagonalCovariance(0.01);
    predictor.setAccelerationInferenceWindowSize(5);

    const int frame_count = 100;
    const int predict_frame_count = 5;

    // static path
    // Path path = produceObstaclePathStatic(frame_count);

    // sine path
    Path path = produceObstaclePathSine(frame_count, 0.1);

    for (int i=0; i<path.size(); i++)
    {
        predictor.observe(path[i]);
        predictor.predict(predict_frame_count);

        for (int j=0; j<predict_frame_count; j++)
        {
            Eigen::Vector3d mu;
            Eigen::Matrix3d sigma;

            predictor.getPredictionResult(j, mu, sigma);

            std::cout << "Prediction at time " << i << " of time " << i+j << std::endl;
            std::cout << "observation = " << path[i].transpose() << std::endl;
            std::cout << "mu = " << mu.transpose() << std::endl;
            std::cout << "sigma = " << std::endl << sigma << std::endl;
            std::cout << std::endl;
        }
    }

    return 0;
}

